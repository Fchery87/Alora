    ...
    <ScreenScroll>
      ...
      <Card style={{ padding: 22, borderRadius: theme.radius.xl, backgroundColor: theme.color.diaperTint }}>
        <View
          style={{
            ...
            paddingVertical: 6,
            borderRadius: 999,
            backgroundColor: theme.color.diaper + "22",
            marginBottom: 16,
          }}
        >
          <Lock color={theme.color.diaper} />
          <AppText variant="caption" weight="semibold" style={{ color: theme.color.diaper }}>
            Private · only you can see this
          </AppText>
        </View>

        <AppText display variant="title" weight="medium" style={{ lineHeight: 30 }}>
        ...
        {saved && (
          <AppText variant="caption" style={{ marginTop: 12, textAlign: "center", color: theme.color.diaper }}>
        ...
        <PressableScale
          ...
          style={{
            ...
            padding: 16,
            borderRadius: theme.radius.lg,
            backgroundColor: theme.color.diaper,
            alignItems: "center",
      ...
      <Card style={{ marginTop: 26, padding: 18 }}>
        ...
        <AppText variant="caption" color="inkSoft" style={{ marginTop: 9, lineHeight: 17 }}>
          Early parenthood is a lot. If you’d like to talk to someone, these free, confidential lines are always here —
          no diagnosis, no judgment.
        </AppText>
        {resources.status === "ready" &&
          resources.data.map((r) => (
            <Resource key={r.id} title={r.title} detail={r.description} badge={r.actionLabel} />
          ))}
      </Card>

      <AppText variant="caption" color="inkFaint" style={{ marginTop: 18, textAlign: "center", lineHeight: 17 }}>
        Alora is not a medical or crisis service and does not provide diagnosis or treatment. If you’re in immediate
  ...
  return (
    ...
    >
      <AppText
        display
        weight="bold"
        style={{ color: theme.color.diaper, fontSize: badge.length > 3 ? 13 : 18, width: 40 }}
      >
        {badge}
      </AppText>
      <View style={{ flex: 1 }}>
