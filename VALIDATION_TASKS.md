...
landed in the working tree. The remaining release blockers are environmental or
human-controlled, not permission to weaken the product contract: apply the new
Supabase migrations, prove the hosted database matrix, provision PowerSync,
run native two-device and accessibility checks, resolve dependency advisories,
and obtain privacy/store approval.
...
| Android export   | Previously pass; rerun before release                         | `npm run export:android` is a CI gate.                                                                                                                                                           |
| iOS export       | Pending this final verification run                           | `npm run export:ios` is a CI gate.                                                                                                                                                               |
...
| Hosted pgTAP     | 52/52 verified by the user through migration `20260814000400` | The five `20260815...` migrations expand the suite to 74 assertions and still need `supabase db push` plus a fresh run.                                                                          |
...
| ------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Release contract and migrations | Code complete           | Canonical additive migration history, no direct membership-management path, and updated runner.                                         | Apply the five new migrations to the linked project.                                     |
...
| Dependencies and CI             | Code complete with risk | Format, typecheck, lint, coverage, exports, backend checks, optional hosted pgTAP, and aggregate gate are wired.                        | Fresh audit, clean runner, and branch-protection check.                                  |
